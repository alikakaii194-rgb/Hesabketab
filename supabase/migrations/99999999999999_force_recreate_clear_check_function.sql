-- This migration forcefully overwrites the clear_check function to fix a state inconsistency in the database.
-- The error 'column "cleared date" does not exist' was occurring despite all migration files being correct.
-- This indicates that a faulty version of the function existed on the server from a past failed migration or manual edit.
-- This script ensures the database function matches the correct definition from the codebase.
...
BEGIN
    ...
    SET status = 'cleared', 
        cleared_date = now(), -- This is the correct column name
        updated_at = now(),
